...
"""List the Atlas USD's physics joints and their drive APIs (ankle focus)."""
import argparse
...
st = Usd.Stage.Open(
    "protomotions/data/assets/usd/atlas/configuration/atlas_flat_physics.usd"
)
...
for p in st.Traverse():
    ...
    if drives:
        driven += 1
    if "Foot" in str(p.GetName()) or "Leg_5" in str(p.GetName()):
