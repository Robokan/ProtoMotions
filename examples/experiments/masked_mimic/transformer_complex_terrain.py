...
#
"""MaskedMimic with Complex Terrain Support.

This experiment adds procedural terrain to MaskedMimic training:
- Slopes, stairs, stepping stones, obstacles
- 16x16 height map observation around the robot
- Recommended: Fine-tune from a flat-ground checkpoint

Usage:
    # Train from scratch (harder)
    python protomotions/train_agent.py \
        --experiment-path examples/experiments/masked_mimic/transformer_complex_terrain.py \
        ...

    # Fine-tune from flat checkpoint (recommended)
    python protomotions/train_agent.py \
        --experiment-path examples/experiments/masked_mimic/transformer_complex_terrain.py \
        --checkpoint results/masked_mimic_g1_phuma/last.ckpt \
        ...
"""
...
def agent_config(
    ...
    # Trunk: Processes VAE latent + self obs + TERRAIN to produce actions
    # KEY CHANGE: Added "terrain" observation for terrain awareness
    trunk_config = SequentialModuleConfig(
        in_keys=["max_coords_obs", "terrain", "historical_previous_actions", "vae_latent"],
